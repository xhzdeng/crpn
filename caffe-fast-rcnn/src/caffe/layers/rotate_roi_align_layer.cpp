...
void RotateROIAlignLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  RotateROIAlignParameter rotate_roi_align_param = this->layer_param_.rotate_roi_align_param();
  CHECK_GT(rotate_roi_align_param.pooled_h(), 0)
      << "pooled_h must be > 0";
  CHECK_GT(rotate_roi_align_param.pooled_w(), 0)
      << "pooled_w must be > 0";
  pooled_height_ = rotate_roi_align_param.pooled_h();
  pooled_width_ = rotate_roi_align_param.pooled_w();
  spatial_scale_ = rotate_roi_align_param.spatial_scale();
